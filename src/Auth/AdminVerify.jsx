...
import { Loader2, ShieldCheck, RefreshCcw, ArrowLeft, ArrowRight } from 'lucide-react';
...
export default function AdminVerify() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/admin/verify-code`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token, code: codeStr })
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || (isArabic ? 'فشل عملية التحقق' : 'Verification failed'));
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(isArabic ? 'فشل إعادة الإرسال' : 'Failed to resend');
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center bg-[#f8fafc] px-4 font-sans ${isArabic ? 'rtl' : 'ltr'}`} dir={isArabic ? 'rtl' : 'ltr'}>
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-blue-100/50 border border-gray-100 p-8 md:p-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* حقول الإدخال تظل LTR دائماً لأنها أرقام */}
